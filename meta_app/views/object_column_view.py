...
class MAObjectColumnListView(ObjectColumnBase, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["object_name"] = f'{dic_object.dic_code} - {dic_object.dic_name} ({dic_object.schema_name}.{dic_object.object_name})'
...
class MAObjectColumnCreateView(ObjectColumnBase, CreateView):
    ...
    def get_form_kwargs(self):
        # Передаем dictionary_id в форму
        kwargs = super().get_form_kwargs()
        kwargs['dictionary_id'] = self.kwargs.get('dictionary_id')
        return kwargs


class MAObjectColumnUpdateView(ObjectColumnBase, UpdateView):
